...
class Trainer:
    def __init__(self, train_ds, test_ds, gpu_id):
        self.n_fft = 400
        self.hop = 100
        ...
        self.gpu_id = gpu_id
        self.epochs_run = 0
        self.snapshot_path = os.path.join(args.save_model_dir, 'snapshot.pt')
    ...
    def train(self):
        ...
        for epoch in range(self.epochs_run, args.epochs):
            ...
            if self.gpu_id == 0:
                ...
                print(f'Epoch {epoch} | Training model saved at {self.snapshot_path}')
                self._save_snapshot(epoch)
            self.scheduler_G.step()
            self.scheduler_D.step()
        os.remove(self.snapshot_path)
        print(f'Removed training snapshot saved from {self.snapshot_path}')

    def _save_snapshot(self, epoch): 
        snapshot = dict()
        snapshot['GENERATOR_STATE'] = self.model.module.state_dict()
        snapshot['DISCRIMINATOR_STATE'] = self.discriminator.module.state_dict()
        snapshot['GENERATOR_OPT_STATE'] = self.optimizer.state_dict()
        snapshot['DISCRIMINATOR_OPT_STATE'] = self.optimizer_disc.state_dict()
        snapshot['SCHEDULER_G_STATE'] = self.scheduler_G.state_dict()
        snapshot['SCHEDULER_D_STATE'] = self.scheduler_D.state_dict()
    ...
    def _load_snapshot(self): 
        ...
        self.model.load_state_dict(snapshot['GENERATOR_STATE'])
        self.discriminator.load_state_dict(snapshot['DISCRIMINATOR_STATE'])
        self.optimizer.load_state_dict(snapshot['GENERATOR_OPT_STATE'])
        self.optimizer_disc.load_state_dict(snapshot['DISCRIMINATOR_OPT_STATE'])
        self.scheduler_G.load_state_dict(snapshot['SCHEDULER_G_STATE'])
        self.scheduler_D.load_state_dict(snapshot['SCHEDULER_D_STATE'])
        self.epochs_run = snapshot['EPOCHS_RUN']
        print(f'Loaded the recent snapshot of epoch {self.epochs_run} from {self.snapshot_path}')

# def main(rank: int, world_size: int, args):
def main():
    dist.init_process_group("nccl" if torch.distributed.is_nccl_available() else "gloo")
    rank = dist.get_rank()
    ...
    if gpu_id == 0:
        ...
        print(f'The available GPUS are : {available_gpus}')
    train_ds, test_ds = dataloader.load_data(
        args.data_dir, args.batch_size, mp.cpu_count(), args.cut_len
    )
